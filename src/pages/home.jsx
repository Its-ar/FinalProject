            ...
            <div className="w-full md:w-2/3 mr-4 flex-1 bg-white p-6 rounded-lg shadow-md">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 mt-4">
                {product
                  ...
                  .map((product) => (
                    <div key={product.id} className={`bg-white p-2 rounded-lg shadow ${cart.length === 0 ? "w-full" : ""}`}>
                      <div className="flex items-center justify-center bg-gray-100 rounded-xl px-2 py-2">
                        <img onClick={() => handleAddToCart(product)} className="h-auto aspect-square object-cover cursor-pointer hover:scale-110 transition-transform duration-500 ease-in-out" src={product.image} alt="Card Image" />
                      </div>
                      <div className="flex flex-col justify-between font-semibold xl:flex-row">
                        <p>{product.name}</p>
